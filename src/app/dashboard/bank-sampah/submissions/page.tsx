...
export default function SubmissionsPage() {
  ...
  const fetchSubmissions = useCallback(
    async (page = 1, statusFilter = "ALL", searchQuery = "") => {
      try {
        ...
        const response = await fetch(`/api/bank-sampah/submissions?${params}`);
  ...
  useEffect(() => {
    if (session?.user?.role === "BANK_SAMPAH_ADMIN") {
      fetchSubmissions(pagination.page, filter, search);
    }
  ...
  const handleFilterChange = (
    ...
    setFilter(newFilter);
    fetchSubmissions(1, newFilter, search);
  };

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    fetchSubmissions(1, filter, search);
  };

  const handlePageChange = (newPage: number) => {
    fetchSubmissions(newPage, filter, search);
  };
      ...
      <div>
        ...
        <form onSubmit={handleSearch} className="mb-4">
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Cari berdasarkan nama produk, deskripsi, atau user..."
              value={search}
